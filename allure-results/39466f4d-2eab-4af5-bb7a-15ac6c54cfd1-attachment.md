...
```
Error: locator.click: Error: strict mode violation: locator('#organizationSize') resolved to 2 elements:
    1) <select id="organizationSize" name="organizationSize" class="corporate-form__select text-grey-3">…</select> aka locator('#header #organizationSize')
    2) <select id="organizationSize" name="organizationSize" class="corporate-form__select text-grey-3">…</select> aka locator('#organizationSize').nth(1)

Call log:
  - waiting for locator('#organizationSize')
          ...
          - iframe [ref=e36]:
            - generic [ref=f5e5]:
              ...
              - generic [ref=f5e9]:
                ...
                - link "Take action." [ref=f5e10] [cursor=pointer]:
...
> 40 |   await page.locator('#organizationSize').click();
     |                                           ^ Error: locator.click: Error: strict mode violation: locator('#organizationSize') resolved to 2 elements:
  41 |   await page.locator('text="10001+"').click()
  42 | 
  43 |   await page.locator('#interestedIn').click();
  44 |   await page.locator('text="Referring someone"').click();
  45 |   const button = page.locator('button[type="submit"]');
  46 |   await expect(button).toBeDisabled();
  47 | });
  48 | 
  49 | 
```